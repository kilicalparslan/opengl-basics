...
private:
    ...
    Mesh processMesh(aiMesh* mesh, const aiScene* scene)
    {
        ...
        for (unsigned int i = 0; i < mesh->mNumVertices; i++)
        {
            ...
            // texture coordinates
            if (mesh->mTextureCoords[0]) // checks mesh contain texture coordinates 
            {
                glm::vec2 vec;
                // a vertex can contain up to 8 different texture coordinates.
                // use models where a vertex can have multiple texture coordinates so always take the first set (0).
